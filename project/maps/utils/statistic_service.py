...
def get_statistic(trip):
    ...
    with contextlib.suppress(Exception):
        stats = models.Statistic.objects.filter(track__trip__pk=trip.pk).filter(
            track__date__range=(trip.start_date, trip.end_date)
        )

        total_km = stats.aggregate(Sum("total_km"))["total_km__sum"]
        total_time = stats.aggregate(Sum("total_time_seconds"))[
            "total_time_seconds__sum"
        ]
